...
class TranscriptionTest(TestCase):
    ...
    def test_transcribe_audio_success(self, mock_groq_client):
        ...
        mock_response.text = (
            "J'ai rêvé d'un oiseau bleu qui volait dans le ciel étoilé"
        )
        ...
        # Vérifications
        self.assertEqual(
            result, "J'ai rêvé d'un oiseau bleu qui volait dans le ciel étoilé"
        )
    ...
    def test_transcribe_audio_logging(self, mock_logger, mock_groq_client):
        ...
        mock_response = MagicMock()
        mock_response.text = (
            "Voici une transcription réussie avec suffisamment de mots"
        )
        mock_groq_client.audio.transcriptions.create.return_value = (
            mock_response
        )
        ...
        success_log_found = any(
            "Transcription" in m
            and ("succès" in m.lower() or "valide" in m.lower())
            for m in info_msgs
        )
        self.assertTrue(
            success_log_found,
            "Le log de réussite de transcription devrait être présent",
        )
...
class EmotionAnalysisTest(TestCase):
    ...
    def test_analyze_emotions_success(
        self, mock_read_file, mock_safe_mistral_call
    ):
        """
        ...
        mock_read_file.return_value = (
            "Tu es un expert en analyse émotionnelle des rêves..."
        )
        mock_response = MagicMock()
        mock_response.choices[0].message.content = json.dumps(
            {"joie": 0.8, "tristesse": 0.1, "surprise": 0.1}
        )
        mock_safe_mistral_call.return_value = mock_response

        emotions, dominant = analyze_emotions(
            "J'ai fait un rêve merveilleux et joyeux"
        )
        ...
        mock_safe_mistral_call.assert_called_once_with(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Tu es un expert en analyse émotionnelle des rêves...",
                },
                {
                    "role": "user",
                    "content": "J'ai fait un rêve merveilleux et joyeux",
                },
            ],
    ...
    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_analyze_emotions_invalid_json_response(
        self, mock_read_file, mock_safe_mistral_call
    ):
        """
    ...
    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_analyze_emotions_partial_response(
        self, mock_read_file, mock_safe_mistral_call
    ):
        """
    ...
    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_analyze_emotions_complex_emotions(
        self, mock_read_file, mock_safe_mistral_call
    ):
        """
        ...
        emotions, dominant = analyze_emotions(
            "Rêve complexe avec nuances émotionnelles"
        )
    ...
    @patch("diary.utils.read_file")
    @patch("diary.utils.logger")
    def test_analyze_emotions_logging(
        self, mock_logger, mock_read_file, mock_safe_mistral_call
    ):
        """
        ...
        emotion_log_found = any(
            ("Émotion dominante" in m and "joie" in m)
            or ("détectée" in m and "joie" in m)
            for m in info_msgs
        )
        self.assertTrue(
            emotion_log_found,
            "Le log de l'émotion dominante devrait être présent",
        )
...
class DreamInterpretationTest(TestCase):
    ...
    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_interpret_dream_success(
        self, mock_read_file, mock_safe_mistral_call
    ):
        """
        ...
        result = interpret_dream(
            "J'ai rêvé d'un oiseau bleu qui volait librement dans le ciel"
        )
        ...
        mock_safe_mistral_call.assert_called_once_with(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Tu es un expert en interprétation des rêves...",
                },
                {
                    "role": "user",
                    "content": "J'ai rêvé d'un oiseau bleu qui volait librement dans le ciel",
                },
            ],
    ...
    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_interpret_dream_with_validation_fix(
        self, mock_read_file, mock_safe_mistral_call
    ):
        """
        ...
        mock_response.choices[0].message.content = json.dumps(
            {
                "Émotionnelle": {
                    "contenu": "Texte émotionnel avec format contenu"
                },
                "Symbolique": {
                    "contenu": "Texte symbolique avec format contenu"
                },
                "Cognitivo-scientifique": {
                    "contenu": "Texte cognitif avec format contenu"
                },
        ...
        self.assertEqual(
            result["Émotionnelle"], "Texte émotionnel avec format contenu"
        )
        self.assertEqual(
            result["Symbolique"], "Texte symbolique avec format contenu"
        )
        self.assertEqual(
            result["Cognitivo-scientifique"],
            "Texte cognitif avec format contenu",
        )
        self.assertEqual(
            result["Freudien"], "Texte freudien avec format contenu"
        )

    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_interpret_dream_mixed_format_response(
        self, mock_read_file, mock_safe_mistral_call
    ):
        """
    ...
    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_interpret_dream_invalid_json(
        self, mock_read_file, mock_safe_mistral_call
    ):
        """
...
class ImageGenerationTest(TestCase):
    ...
    def test_generate_image_success(self, mock_read_file, mock_mistral_client):
        ...
        dream = Dream.objects.create(
            user=self.user, transcription="Rêve avec génération d'image"
        )

        # Test de la génération
        result = generate_image_from_text(
            self.user, "Un oiseau bleu volant dans un ciel étoilé", dream
        )
    ...
    @patch("diary.utils.mistral_client")
    @patch("diary.utils.read_file")
    def test_generate_image_quota_exceeded(
        ...
        mock_mistral_client.beta.agents.create.side_effect = Exception(
            "quota_exceeded"
        )

        dream = Dream.objects.create(
            user=self.user, transcription="Rêve sans image"
        )
    ...
    @patch("diary.utils.mistral_client")
    @patch("diary.utils.read_file")
    def test_generate_image_no_file_generated(
        self, mock_read_file, mock_mistral_client
    ):
        """
        ...
        dream = Dream.objects.create(
            user=self.user, transcription="Rêve sans génération"
        )
    ...
    @patch("diary.utils.read_file")
    @patch("diary.utils.logger")
    def test_generate_image_logging(
        self, mock_logger, mock_read_file, mock_mistral_client
    ):
        """
        ...
        dream = Dream.objects.create(
            user=self.user, transcription="Rêve logging"
        )
        ...
        self.assertTrue(
            any(
                "erreur" in m.lower() and "image" in m.lower()
                for m in error_msgs
            ),
...
class SafeMistralCallTest(TestCase):
    ...
    def test_fallback_chain_mistral_large_latest(self, mock_mistral_client):
        ...
        result = safe_mistral_call(
            "mistral-large-latest", messages, "Test fallback"
        )
    ...
    def test_fallback_chain_mistral_medium(self, mock_mistral_client):
        ...
        messages = [{"role": "user", "content": "test"}]
        result = safe_mistral_call(
            "mistral-medium", messages, "Test fallback medium"
        )
    ...
    def test_fallback_different_error_types(self, mock_mistral_client):
        ...
        for error_type in error_types_that_should_fallback:
            with self.subTest(error_type=error_type):
                ...
                messages = [{"role": "user", "content": "test"}]
                result = safe_mistral_call(
                    "mistral-large-latest", messages, f"Test {error_type}"
                )

                self.assertEqual(result, mock_response)
                self.assertEqual(
                    mock_mistral_client.chat.complete.call_count, 2
                )
    ...
    def test_critical_error_no_fallback(self, mock_mistral_client):
        ...
        for error_type in critical_errors:
            with self.subTest(error_type=error_type):
                ...
                self.assertEqual(
                    mock_mistral_client.chat.complete.call_count, 1
                )
    ...
    def test_fallback_complete_failure(self, mock_mistral_client):
        ...
        messages = [{"role": "user", "content": "test"}]
        result = safe_mistral_call(
            "mistral-large-latest", messages, "Test échec complet"
        )
    ...
    def test_fallback_logging(self, mock_logger, mock_mistral_client):
        ...
        messages = [{"role": "user", "content": "test"}]
        result = safe_mistral_call(
            "mistral-large-latest", messages, "Test logging"
        )
        ...
        self.assertTrue(
            any(
                "[Test logging]" in m
                and "Démarrage avec" in m
                and "mistral-large-latest" in m
                for m in info_msgs
            ),
        ...
        self.assertTrue(
            any(
                "QUOTA ATTEINT" in m and "mistral-large-latest" in m
                for m in warn_msgs
            ),
        ...
        self.assertTrue(
            any(
                "Fallback utilisé" in m and "mistral-medium" in m
                for m in warn_msgs
            ),
    ...
    def test_fallback_performance_tracking(self, mock_mistral_client):
        ...
        messages = [{"role": "user", "content": "test"}]
        result = safe_mistral_call(
            "mistral-large-latest", messages, "Test performance"
        )
    ...
    def test_fallback_chain_configuration(self):
        ...
        with patch("diary.utils.mistral_client") as mock_mistral:
            mock_mistral.chat.complete.side_effect = Exception(
                "quota_exceeded"
            )

            messages = [{"role": "user", "content": "test"}]
            result = safe_mistral_call(
                "open-mistral-7b", messages, "Test no fallback"
            )
    ...
    def test_fallback_with_json_response_format(self, mock_mistral_client):
        ...
        messages = [{"role": "user", "content": "test"}]
        result = safe_mistral_call(
            "mistral-large-latest", messages, "Test JSON format"
        )
    ...
    def test_safe_mistral_call_timeout_error(self, mock_mistral_client):
        ...
        result = safe_mistral_call(
            "mistral-large-latest", messages, "Test TimeoutError"
        )
...
class AIFunctionsIntegrationTest(TestCase):
    ...
    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_emotion_analysis_with_fallback_integration(
        self, mock_read_file, mock_safe_mistral
    ):
        """
        ...
        mock_safe_mistral.assert_called_once_with(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Prompt système d'analyse émotionnelle",
                },
    ...
    @patch("diary.utils.safe_mistral_call")
    @patch("diary.utils.read_file")
    def test_interpretation_with_fallback_integration(
        self, mock_read_file, mock_safe_mistral
    ):
        """
        ...
        mock_safe_mistral.assert_called_once_with(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Prompt système d'interprétation",
                },
                {
                    "role": "user",
                    "content": "Rêve à interpréter avec fallback",
                },
            ],
            operation="Interprétation",
        )
    ...
    def test_complete_ai_workflow_with_fallbacks(self):
        ...
        with patch("diary.utils.groq_client") as mock_groq, patch(
            "diary.utils.safe_mistral_call"
        ) as mock_mistral:

            # Configuration pour transcription
            mock_transcription = MagicMock()
            mock_transcription.text = "Rêve d'intégration IA très détaillé"
            mock_groq.audio.transcriptions.create.return_value = (
                mock_transcription
            )
            ...
            self.assertEqual(
                transcription, "Rêve d'intégration IA très détaillé"
            )
            self.assertIsNotNone(emotions)
            self.assertEqual(dominant[0], "joie")
...
class SafeMistralCallTest(TestCase):
    ...
    def test_fallback_critical_error_no_retry(self, mock_mistral_client):
        ...
        with self.assertRaises(Exception):
            safe_mistral_call(
                "mistral-large-latest", messages, "Test erreur critique"
            )
...
class AIResponseValidationTest(TestCase):
    ...
    def test_emotion_response_validation(self):
        ...
        self.assertTrue(
            all(isinstance(v, (int, float)) for v in valid_response.values())
        )
                ...
                if invalid_response:
                    try:
                        has_numeric_values = all(
                            ...
                            for v in invalid_response.values()
                        )
                        if has_numeric_values:
                            has_valid_range = all(
                                0 <= v <= 1 for v in invalid_response.values()
                            )
                            sum_valid = sum(invalid_response.values()) <= 1.1
                            is_valid = has_valid_range and sum_valid
                            self.assertFalse(
                                is_valid,
                                f"Response should be invalid: {invalid_response}",
                            )
                        else:
                            self.assertFalse(
                                has_numeric_values,
                                f"Should detect non-numeric values: {invalid_response}",
                            )
        ...
        for problematic in problematic_responses:
            with self.subTest(response=problematic):
                ...
                needs_correction = not all(
                    key in problematic for key in expected_keys
                ) or not all(
                    isinstance(v, str) and len(v) > 0
                    for v in problematic.values()
                )
