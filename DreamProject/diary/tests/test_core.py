...
def _collect_sse_events(response):
    ...
    for chunk in response.streaming_content:
        try:
            ...
            # Log spécifique pour problèmes d'encodage
            logger.debug(f"Chunk SSE non décodable ignoré: {e}")
            continue
        except json.JSONDecodeError as e:
            # Log spécifique pour JSON malformé
            logger.debug(f"JSON SSE malformé ignoré: {payload[:30]}... - {e}")
            continue
        except Exception as e:
            # Log pour erreurs inattendues avec plus de contexte
            logger.warning(
                f"Erreur parsing chunk SSE inattendue: {e} - chunk: {chunk[:50]}"
            )
            continue
    return events
...
class CoreModelTest(TestCase):
    ...
    def test_dream_creation_basic(self):
        ...
        dream = Dream.objects.create(
            user=self.user, transcription="Test critique de création de rêve"
        )
    ...
    def test_emotions_property_core(self):
        ...
        """
        dream = Dream.objects.create(
            user=self.user, transcription="Test emotions"
        )
    ...
    def test_interpretation_property_core(self):
        ...
        """
        dream = Dream.objects.create(
            user=self.user, transcription="Test interpretation"
        )
    ...
    def test_short_transcription_core(self):
        ...
        short_dream = Dream.objects.create(
            user=self.user, transcription="Court"
        )
        ...
        self.assertEqual(
            long_dream.short_transcription, long_text[:100] + "..."
        )
...
class CoreUtilsTest(TestCase):
    ...
    def test_softmax_function_core(self):
        ...
        """
        raw_scores = {"joie": 2.0, "tristesse": 1.0, "peur": 0.5}
...
class CoreErrorHandlingTest(TestCase):
    ...
    def test_none_values_handling(self):
        ...
        """
        dream = Dream.objects.create(
            user=self.user, transcription="Test None values"
        )
