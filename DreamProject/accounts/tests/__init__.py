...
def print_test_summary():
    ...
    print(
        "python manage.py test accounts.tests                    # Tous les tests"
    )
    print(
        "python manage.py test accounts.tests.test_core          # Tests critiques"
    )
    print(
        "python manage.py test accounts.tests --parallel         # Tests parallèles"
    )
...
if os.environ.get("DJANGO_SETTINGS_MODULE") and not os.environ.get(
    ...
    print(
        "6 fichiers de tests prêts - ~98% de couverture - ~1m35s d'exécution"
    )
